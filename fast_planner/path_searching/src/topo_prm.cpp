...
void TopologyPRM::findTopoPaths(Eigen::Vector3d start, Eigen::Vector3d end,
  ...
  graph_time = (rclcpp::Clock(RCL_ROS_TIME).now() - t1).seconds();
  ...
  short_time = (rclcpp::Clock(RCL_ROS_TIME).now() - t1).seconds();
  ...
  prune_time = (rclcpp::Clock(RCL_ROS_TIME).now() - t1).seconds();
  ...
  select_time = (rclcpp::Clock(RCL_ROS_TIME).now() - t1).seconds();
...
list<GraphNode::Ptr> TopologyPRM::createGraph(Eigen::Vector3d start, Eigen::Vector3d end) {
  ...
  while (sample_time < max_sample_time_ && sample_num < max_sample_num_) {
    ...
    if (dist <= clearance_) {
      sample_time += (rclcpp::Clock(RCL_ROS_TIME).now() - t1).seconds();
      continue;
    }
...
vector<Eigen::Vector3d> TopologyPRM::discretizePath(vector<Eigen::Vector3d> path) {
  ...
  if (path.size() < 2) {
    RCLCPP_ERROR(rclcpp::get_logger("TopologyPRM::discretizePath"), "what path? ");
    return dis_path;
  }
