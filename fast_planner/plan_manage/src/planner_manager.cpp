...
bool FastPlannerManager::kinodynamicReplan(Eigen::Vector3d start_pt, Eigen::Vector3d start_vel,
  ...
  local_data_.start_time_ = rclcpp::Clock(RCL_ROS_TIME).now();
  ...
  t_search = (rclcpp::Clock(RCL_ROS_TIME).now() - t1).seconds();
  ...
  t_opt = (rclcpp::Clock(RCL_ROS_TIME).now() - t1).seconds();
  ...
  if (tn / to > 3.0) RCLCPP_ERROR(rclcpp::get_logger("kino_replan"), "reallocate error.");
...
bool FastPlannerManager::planGlobalTraj(const Eigen::Vector3d& start_pos) {
  ...
  RCLCPP_INFO(rclcpp::get_logger("Global trajectory"), "global trajectory generated.");
...
bool FastPlannerManager::topoReplan(bool collide) {
  ...
  if (!collide) {  // simply truncate the segment and do nothing
    ...
    if (colli_start.size() == 1 && colli_end.size() == 0) {
      RCLCPP_WARN(rclcpp::get_logger("Replanning"), "Init traj ends in obstacle, no replanning.");
      local_data_.position_traj_ = init_traj;
      ...
      RCLCPP_WARN(rclcpp::get_logger("Replanning"),"[Topo]: ---------");
      plan_data_.clearTopoPaths();
...
void FastPlannerManager::refineTraj(NonUniformBspline& best_traj, double& time_inc) {
  ...
  RCLCPP_WARN_STREAM(rclcpp::get_logger("Replanning"), "[Refine]: cost " << (rclcpp::Clock(RCL_ROS_TIME).now() - t1).seconds()
...
void FastPlannerManager::optimizeTopoBspline(double start_t, double duration,
  ...
  if (guide_pt.size() != int(ctrl_pts.rows()) - 6) RCLCPP_WARN(rclcpp::get_logger("Optimization"), "what guide");
  ...
  RCLCPP_INFO(rclcpp::get_logger("Optimization"),"optimization %d cost %lf, %lf, %lf seconds.", traj_id, tm1, tm2, tm3);
}
...
void FastPlannerManager::planYaw(const Eigen::Vector3d& start_yaw) {
  RCLCPP_INFO(rclcpp::get_logger("planYaw"), "plan yaw");
  ...
  std::cout << "plan heading: " << (rclcpp::Clock(RCL_ROS_TIME).now() - t1).seconds() << std::endl;
}
